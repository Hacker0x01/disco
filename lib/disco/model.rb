module Disco
  module Model
    def has_recommended(name, class_name: nil)
      class_name ||= base_class.to_s.singularize.camelize
      ...
      class_eval do
        ...
        define_method("update_recommended_#{name}") do |items|
          now = Time.now
          items = items.map { |item| {subject_type: class_name, subject_id: id, item_type: class_name, item_id: item[:item_id], context: name, score: item[:score], created_at: now, updated_at: now} }
